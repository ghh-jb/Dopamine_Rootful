...
void jbinfo_initialize_hardcoded_offsets(void)
{
	...
	gSystemInfo.kernelStruct.task.jop_disabled = 0x348;	
	gSystemInfo.kernelStruct.task.firstThread_jop_disabled = 0x15E;
	...
	gSystemInfo.kernelStruct.pmap.jop_disabled = 0xC4 + pmapEl2Adjust;
		...
		if (strcmp(xnuVersion, "21.2.0") >= 0) { // iOS 15.2+
			...
			if (strcmp(xnuVersion, "21.4.0") >= 0) { // iOS 15.4+
				...
				if (strcmp(xnuVersion, "22.0.0") >= 0) { // iOS 16+
					...
					gSystemInfo.kernelStruct.pmap.jop_disabled = 0xC8 + pmapEl2Adjust;
#else
