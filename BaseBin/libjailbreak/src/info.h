...
struct system_info {
	...
	struct {
		...
		struct {
			...
			uint32_t mach_trap_filter_mask;
			uint32_t mach_kobj_filter_mask;
			uint32_t jop_disabled;
			uint32_t firstThread_jop_disabled;
		} task;
...
#define KERNEL_STRUCTS_ITERATE(ctx, iterator) \
	...
	iterator(ctx, kernelStruct.task.jop_disabled); \
	iterator(ctx, kernelStruct.task.firstThread_jop_disabled); \
	...
	iterator(ctx, kernelStruct.pmap.jop_disabled); \
