			...
			if (vm_map) {
				...
				if (pmap) {
					...
					if (emulateFully) {
						...
						proc_csflags_clear(proc, CS_EXEC_SET_ENFORCEMENT | CS_EXEC_SET_KILL | CS_EXEC_SET_HARD | CS_REQUIRE_LV | CS_ENFORCEMENT | CS_RESTRICT | CS_KILL | CS_HARD | CS_FORCED_LV | CS_FORCED_LV | CS_CHECK_EXPIRATION | CS_RUNTIME);
						proc_csflags_set(proc, CS_DEBUGGED | CS_INVALID_ALLOWED | CS_GET_TASK_ALLOW | CS_VALID | CS_SIGNED | CS_ADHOC | CS_PLATFORM_BINARY | CS_NVRAM_UNRESTRICTED | CS_ENTITLEMENTS_VALIDATED);
