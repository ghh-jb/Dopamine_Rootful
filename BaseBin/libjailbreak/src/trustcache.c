...
#include <os/log.h> // ohhh i feel this will end bad

void dbglog(char *format, ...) {
	/*
    va_list args;
    va_start(args, format);
    FILE* fd = fopen("/tmp/trustcache.log", "a");
    char buffer[256];
...
{
	dbglog("[TCDBG] trustcache_list_remove: start\n");
	if (!tcKaddr) return -1;
...
{
	dbglog("[TCDBG] _is_jb_trustcache: start\n");
	...
	if (file == jbTcFile) {
		...
		bool ret = (kread64(tcKaddr + offsetof(jb_trustcache, magic)) == JB_MAGIC);
		dbglog("[TCDBG] _is_jb_trustcache: %i (kread + magic)\n", ret);
		return ret;
	}
	dbglog("[TCDBG] _is_jb_trustcache: false (file)\n");
	return false;
}
...
{
	dbglog("[TCDBG] wiping all cached cdhashes\n");
...
{
	dbglog("[TCDBG] add_entries: start\n");
	...
	while (remainingEntryCount > 0) {
		...
		for (uint32_t i = 0; i < entryCountToInsert; i++) {
			...
			// jbTc->file.entries[freeJbTcCurrentLength+i].constraintCategory = 0;
		}
		jbTc->file.length += entryCountToInsert;
		_trustcache_file_sort(&jbTc->file);
		dbglog("[TCDBG] add_entries: write\n");
...
int jb_trustcache_add_cdhashes(cdhash_t *hashes, uint32_t hashCount)
{
	...
	dbglog("[TCDBG] add_cdhashes: hash count %u\n", hashCount);
	for (int i = 0; i < hashCount; i++) {
		dbglog("[TCDBG] add_cdhashes: copying %i\n", i);
		memcpy(entries[i].hash, hashes[i], sizeof(cdhash_t));
		entries[i].hash_type = 2;
		entries[i].flags = 0;
		// entries[i].constraintCategory = 0;
		// entries[i].reserved0 = 0;
	}
	dbglog("[TCDBG] add_cdhashes: adding entries\n");
...
{
	dbglog("[TCDBG] sending debug info to XPC\n");
...
{
	dbglog("[TCDBG] printing debug info\n");
	__block int i = 0;
	_jb_trustcache_enumerate(^(uint64_t jbTcKaddr, bool *stop) {
		uuid_t uuid;
		kreadbuf(jbTcKaddr + offsetof(jb_trustcache, file.uuid), (void *)uuid, sizeof(uuid));
...
{
	dbglog("[TCDBG] trustcache_file_upload: start\n");
...
int trustcache_file_build_from_cdhashes(cdhash_t *CDHashes, uint32_t CDHashCount, trustcache_file_v1 **tcOut)
{
	...
	for (uint32_t i = 0; i < CDHashCount; i++) {
		...
		file->entries[i].hash_type = 1;
		file->entries[i].flags = 0;
		// file->entries[i].constraintCategory = 0;
		// file->entries[i].reserved0 = 0;
	}
