...
typedef struct {
    char* _OVERLAY_CONFIG_PATH[PATH_MAX];
    char* _OVERLAY_STORE_PREFIX[PATH_MAX];
    char* _LIBJAILBREAK_PATH[PATH_MAX];
} jbroot_items;
...
#define OVERLAY_CONFIG_PATH getItemInJBROOT("/overlays/overlay_list.conf")
#define OVERLAY_STORE_PREFIX getItemInJBROOT("/overlays")
#define LIBJAILBREAK_PATH getItemInJBROOT("/basebin/libjailbreak.dylib")
...
int mount_unsandboxed(const char *type, const char *dir, int flags, void *data) {
    __block int ret = 0;
    if (access(getItemInJBROOT("/.installed_dopamine"), F_OK) == 0) {
...
static kern_return_t write_overlay_config(const overlay_info_t *overlay) {
    ...
    kern_return_t ret = ensure_directory_exists(OVERLAY_STORE_PREFIX);
    if (ret != 0) {
        return ret;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("================================================\n");
        // jbrootpath();
        // printf_success("[+] libjailbreak: %s\n", getItemInJBROOT("/basebin/libjailbreak.dylib"));
        // printf_success("[+] overlays_store: %s\n", getItemInJBROOT("/overlays"));
        // printf_success("[+] overlays_config: %s\n", getItemInJBROOT("/overlays/overlay_list.conf"));
        // printf_success("lj: %s\n", LIBJAILBREAK_PATH);
        // printf_success("overstore: %s\n", OVERLAY_STORE_PREFIX);
        // printf_success("overconf: %s\n", OVERLAY_CONFIG_PATH);
        return 1;
    }
